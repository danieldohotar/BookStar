...
public class JDBCBooksDAO implements BooksDAO {
    ...
    public Collection<Books> searchByTitle(String query) {
        ...
        try (ResultSet rs = connection.createStatement()
                .executeQuery(String.format("select * from books "
        ...
        try {
            ...
            if (model.getId() > 0) {
                ps = connection.prepareStatement(
                        "update books set title=?, author=?, genre=?, language=?, year=?, price=?, photopath=?, single=?, comment=?"
                                + "where id = ? returning id");

            } else {
                ps = connection.prepareStatement(
                        "insert into books (title, author, genre, language, year, price, photopath) "
                                + "values (?, ?, ?, ?, ?, ?, ?, ?, ? ) returning id");
            ...
            ps.setString(8, model.getSingle());
            ps.setString(9, model.getComment());
    ...
    private Books extractBooks(ResultSet rs) throws SQLException {
        ...
        books.setSingle(rs.getString("single"));
        books.setComment(rs.getString("comment"));
